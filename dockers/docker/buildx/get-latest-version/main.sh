...
function main {
    ...
    echo "LATEST_VERSION=\"$latest_version\"" > /working-env/docker/buildx/configs/latest-version.cfg
    if [ $? -ne 0 ]; then
...
function getVersion {
    ...
    while true; do
        api_url="https://api.github.com/repos/docker/buildx/releases?page=$page"
