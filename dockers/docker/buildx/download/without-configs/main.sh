...
function main {
    echo "Starting Docker buildx download script..."
    ...
    for what_to_download in "${VERSIONS[@]}"; do
        ...
        arch="${parts[1]}"
        version="${parts[2]}"
        echo "Installing Docker buildx version: $version for OS: $os and Architecture: $arch"
        # Here you would add the commands to install the specific version of Docker buildx
        ...
        echo "Downloading Docker buildx version: $version"
        (wget -c "https://github.com/docker/buildx/releases/download/$version/buildx-$version.$os-$arch" -O "$file_name")
        [ $? -ne 0 ] && exit 1
...
function getFileVersionName {
    ...
    local version="$3"
    local dir_name="$4"
    echo "$dir_name/buildx-${version}.${os}-${arch}"
    exit 0
}
