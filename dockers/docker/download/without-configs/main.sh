...
function main {
    echo "Starting Docker download script..."
    ...
    for what_to_download in "${VERSIONS[@]}"; do
        ...
        arch="${parts[1]}"
        version="${parts[2]}"
        echo "Installing Docker version: $version for OS: $os and Architecture: $arch"
        # Here you would add the commands to install the specific version of Docker
        ...
        echo "Downloading Docker version: $version"
        (wget -c "https://download.docker.com/${os}/static/stable/${arch}/docker-${version}.tgz" -O "$file_name")
        [ $? -ne 0 ] && exit 1
...
function getFileVersionName {
    ...
    local version="$3"
    local dir_name="$4"
    echo "$dir_name/${os}-${arch}-${version}.tgz"
    exit 0
}
